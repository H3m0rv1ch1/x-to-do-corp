...
const RightSidebar: React.FC = () => {
  ...
  return (
    <aside className="sticky top-0 h-screen hidden xl:flex flex-col space-y-4 p-4 w-[350px] flex-shrink-0">
      <div className="relative">
        <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
          <Search className="w-5 h-5 text-[rgba(var(--foreground-secondary-rgb))]" />
        </div>
        <input
          ...
          className="w-full bg-black border border-[rgba(var(--border-primary-rgb))] text-[rgba(var(--foreground-primary-rgb))] rounded-full py-2 pl-10 pr-4 focus:outline-none focus:ring-2 focus:ring-[rgba(var(--accent-rgb))]"
        />
      </div>
            ...
            tasksToShow.map((task) => (
              <div key={task.id} className="group cursor-pointer rounded-lg -m-2 p-2 hover:bg-[rgba(var(--foreground-primary-rgb),0.05)] transition-colors duration-200" onClick={handleShowMore}>
                <div className="flex justify-between items-center">
                    ...
                    {task.isImportant && <Star className="w-4 h-4 text-[rgba(var(--warning-rgb))]" fill="currentColor" />}
                </div>
