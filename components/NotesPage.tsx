...
import { PenLine as PenIcon, StickyNote as NoteIcon, Pin as PinIcon, Search as SearchIcon, MoreHorizontal as MoreIcon, Trash2 as TrashIcon, Check as CheckIcon } from 'lucide-react';
  ...
  return (
    ...
    >
      <div className="flex justify-between items-start">
        ...
        {note.isPinned && <PinIcon className="w-4 h-4 text-[rgba(var(--accent-rgb))] flex-shrink-0 mt-1" />}
      </div>
         ...
         <header className="sticky top-0 z-20 bg-[rgba(var(--background-primary-rgb),0.8)] backdrop-blur-md border-b border-[rgba(var(--border-primary-rgb))]">
            <div className="flex items-center justify-between p-4 h-16">
              ...
              <div className="flex items-center space-x-2">
                   ...
                   {isMoreMenuOpen && (
                     <div className="absolute top-full right-0 mt-2 w-56 bg-[rgba(var(--background-primary-rgb))] rounded-lg shadow-lg border border-[rgba(var(--border-primary-rgb))] z-10 py-1 animate-fade-in">
                       ...
                       {sortOptions.map(opt => (
                         ...
                         >
                           <span>{opt.label}</span>
                           {notesSortOrder === opt.key && <CheckIcon className="w-4 h-4 text-[rgba(var(--accent-rgb))]" />}
                         </button>
                       ))}
                ...
                >
                  <PenIcon className="w-6 h-6 text-[rgba(var(--foreground-primary-rgb))]" />
                </button>
              </div>
            </div>
            <div className="px-4 pb-2 border-b border-[rgba(var(--border-primary-rgb))]">
               <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full bg-[rgba(var(--background-secondary-rgb))] text-[rgba(var(--foreground-primary-rgb))] rounded-full py-2 pl-10 pr-4 focus:outline-none focus:ring-1 focus:ring-[rgba(var(--accent-rgb))]"
                  />
                </div>
            </div>
        </header>
            ...
            {filteredNotes.length > 0 ? (
                ...
                <div className="text-center p-8 text-[rgba(var(--foreground-secondary-rgb))]">
                    <div className="max-w-xs mx-auto">
                      ...
                      <p>{notesSearchQuery ? 'Try a different search term.' : <>Click the <PenIcon className="w-4 h-4 inline-block -mt-1"/> icon to create your first note.</>}</p>
                    </div>
                </div>
            )}
        </div>
