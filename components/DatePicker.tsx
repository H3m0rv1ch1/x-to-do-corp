  ...
  return (
    <div className="absolute top-full left-0 mt-2 w-72 bg-[rgba(var(--background-primary-rgb))] rounded-lg shadow-lg border border-[rgba(var(--border-primary-rgb))] p-4 z-30 animate-fade-in">
      <div className="flex justify-between items-center mb-4">
        <button onClick={handlePrevMonth} className="p-1 rounded-full hover:bg-[rgba(var(--background-tertiary-rgb))]">
          <ChevronLeftIcon className="w-5 h-5 text-[rgba(var(--foreground-secondary-rgb))]" />
        </button>
        <span className="font-bold text-[rgba(var(--foreground-primary-rgb))]">
          {currentMonth.toLocaleString('default', { month: 'long', year: 'numeric' })}
        </span>
        <button onClick={handleNextMonth} className="p-1 rounded-full hover:bg-[rgba(var(--background-tertiary-rgb))]">
          <ChevronRightIcon className="w-5 h-5 text-[rgba(var(--foreground-secondary-rgb))]" />
        </button>
      </div>
