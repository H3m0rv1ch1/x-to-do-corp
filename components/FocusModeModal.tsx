...
import { X as CloseIcon, Play as PlayIcon, Pause as PauseIcon, RotateCcw as ResetIcon, Settings as SettingsIcon, Check as CheckIcon, Maximize as FullScreenIcon, Minimize as ExitFullScreenIcon, Volume2 as VolumeUpIcon, VolumeX as VolumeOffIcon } from 'lucide-react';
import { SessionMode } from '../contexts/AppContext';
                            ...
                            <div>
                             ...
                             <div>
                                ...
                                <div className="flex items-center space-x-2 mt-2">
                                    <select value={focusSettings.sound} onChange={e => updateFocusSettings({ sound: e.target.value })}>
                                    ...
                                    <button onClick={() => updateFocusSettings({ volume: focusSettings.volume > 0 ? 0 : 0.5 })} className="p-2">
                                        {focusSettings.volume > 0 ? <VolumeUpIcon className="w-5 h-5"/> : <VolumeOffIcon className="w-5 h-5"/>}
                                    </button>
                                </div>
                ...
                <footer className="relative z-10 flex items-center justify-center p-4 flex-shrink-0">
                    <div className="flex items-center space-x-2">
                         {Object.entries(AMBIENT_SOUNDS).map(([key, {label}]) => key !== 'none' && (
                            <button key={key} onClick={() => updateFocusSettings({ sound: focusSettings.sound === key ? 'none' : key })} className={`px-3 py-1 text-xs rounded-full border transition-colors ${focusSettings.sound === key ? 'bg-[rgba(var(--accent-rgb),0.2)] border-[rgba(var(--accent-rgb))] text-[rgba(var(--accent-rgb))]' : 'border-transparent text-[rgba(var(--foreground-secondary-rgb))] hover:bg-[rgba(var(--background-tertiary-rgb))]'}`}>{label}</button>
                         ))}
                    </div>
                </footer>
